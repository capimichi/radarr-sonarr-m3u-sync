...
interface SeasonProps {
    seriesId?: number; // Optional, if needed for context
  season: Season;
}

const SeasonItem: React.FC<SeasonProps> = ({ season, seriesId }) => {
  const { seriesService, downloadService } = useServices();
  ...
  const handleDownload = async (episodeId: number) => {
    ...
    // Default download path if not specified
    const path = downloadPaths[episodeId] || `/Users/michele/Downloads/episode_${episodeId}.mp4`;
                      ...
                      <div className="mt-3 space-y-2">
                        ...
                        <div className="flex items-center space-x-2">
                          <input
                            ...
                            value={downloadPaths[episode.id] || ''}
